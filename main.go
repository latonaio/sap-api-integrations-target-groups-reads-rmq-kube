...
import (
	sap_api_caller "sap-api-integrations-target-groups-reads-rmq-kube/SAP_API_Caller"
	sap_api_input_reader "sap-api-integrations-target-groups-reads-rmq-kube/SAP_API_Input_Reader"
	"sap-api-integrations-target-groups-reads-rmq-kube/config"
...
func callProcess(caller *sap_api_caller.SAPAPICaller, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	iD := extractData(msg.Data())
	accepter := getAccepter(msg.Data())
	caller.AsyncGetTargetGroup(iD, accepter)
	return nil
}

func extractData(data map[string]interface{}) (iD string) {
	sdc := sap_api_input_reader.ConvertToSDC(data)
	iD = sdc.TargetGroupCollection.ID
	return
}
